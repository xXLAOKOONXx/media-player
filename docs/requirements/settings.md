...
1. **Stats database folder** (text input with Browse button)
   - Default: empty string (uses the backend executable directory as default location)
   - Description: "Folder where the media-player-stats.db file will be stored. Leave empty to use the default location (the directory containing the backend executable)."
   - Browse button: Opens folder browser dialog
   - When folder is set or left empty:
     - Application creates `media-player-stats.db` in the specified folder (or the backend executable directory if empty)
