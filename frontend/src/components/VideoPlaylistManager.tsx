...
import './VideoPlaylistManager.css';
...
const VideoPlaylistManager = ({ currentUser }: VideoPlaylistManagerProps) => {
  ...
  const loadPlaylists = async (folderId: number) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/video/playlists/${folderId}/files`);
      const data = await response.json();
      setPlaylists(data);
    } catch (err) {
  ...
  const handleAddFolder = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await fetch(`${API_BASE_URL}/api/video/playlists`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteFolder = async (folderId: number) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/video/playlists/${folderId}`, {
        method: 'DELETE'
      });
        ...
        <div className="folders-list">
          {playlistFolders.length === 0 ? (
            <div className="empty-state">
              <p>No video playlist folders configured</p>
      ...
      {selectedFolder && (
        <div className="card">
          ...
          <div className="playlists-list">
            {playlists.length === 0 ? (
              <div className="empty-state">
                <p>No video playlists found in this folder</p>
              </div>
            ) : (
